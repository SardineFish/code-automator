...
test("startService boots with extension providers only when gh is absent", async (t) => {
  const dir = createTempDir(t, "gao-start-service-no-gh-");
  const extensionPath = path.join(dir, "example-extension.js");
  const config = createServiceConfig();
...
`
  ...
  const baseUrl = `http://127.0.0.1:${address.port}`;
  const extensionResult = await fetch(`${baseUrl}/custom-hook`, { method: "POST" });
  const githubResult = await fetch(`${baseUrl}/gh-hook`, { method: "GET" });
  ...
  assert.equal(githubResult.status, 404);
